...
public class S2SEventHandlerJobListener extends JobExecutionListenerSupport {

    private static final Logger log = LoggerFactory.getLogger(S2SEventHandlerJobListener.class);
    ...
    public void afterJob(JobExecution jobExecution) {
        if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
            log.info("S2SEventHandlerJob job complete successfully");
        } else if (jobExecution.getStatus() == BatchStatus.FAILED) {
            emailWriter.sendJobFailureMessage(mailHelper.getMailRequest(mailRequest));
            log.info("S2SEventHandlerJob job failed");
        } else if (jobExecution.getStatus() == BatchStatus.STOPPED) {
            log.info("S2SEventHandlerJob job stopped");
        }
    }
}
